...
try {
    ...
    // 1. Dijkstra implementation in PHP with congestion and step-free penalties
    $distances = [];
    $previous = [];
    ...
    while (!$queue->isEmpty()) {
        ...
        foreach ($graph[$u] as $v => $baseWeight) {
            ...
            if ($stepFree && !$hasElevator) {
                ...
                // Note: Restrooms, Sections, Gates, and Transits can contain stairs.
            ...
            if ($alt < $distances[$v]) {
                $distances[$v] = $alt;
                $previous[$v] = $u;
                // SplPriorityQueue is max-heap by default, so we insert negative priorities
                $queue->insert($v, -$alt);
            }
        }
    }
